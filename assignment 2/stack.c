...
struct stack* stack_create() {
	...
	struct stack* newStack = malloc(sizeof(struct stack)); //Allocates memory to stack
	newStack->list = list_create(); //Allocates memory to linked list
...
void stack_free(struct stack* stack) {
	 ...
	 * FIXME:
	 */
	list_free(stack->list); //Frees memory from list
	free(stack); //frees memory from struct
	return;
}
...
void stack_push(struct stack* stack, void* val) {
	 ...
	 * FIXME:
	 */
	list_insert(stack->list, val); //Inserts node at head of list
	return;
}
...
void* stack_top(struct stack* stack) {
	 ...
	 * FIXME:
	 */
	void** val = list_return(stack->list); //Returns head node value
	return val;
}
...
void* stack_pop(struct stack* stack) {
	 ...
	 * FIXME:
	 */
	void** val = list_return(stack->list); //Copies head node value
	list_remove_one(stack->list); //Removes head node value
	return val;
}
